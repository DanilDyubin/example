...
function Portal({ children, wrapperId = 'portal-wrapper' }) {
  ...
  useLayoutEffect(() => {
    ...
    if (!element) {
      created = true; // эта переменная создается для проверки существует ли элемент на странице или он был создан с помощью createWrapperAndAppendToBody, потому что во втором случае его нужно будет при закрытии удалить со страницы
